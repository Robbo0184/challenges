...
const allAnimalsInEuropeWeighLessThanOnehundred = animals.filter(animal => 
   animal.continents.includes("Europe")).every(animal => animal.weight < 100)
...
// }).map(animal => animal.weight).reduce((a, b) => a.weight + b.weight);

const allAnimalsInAfrica = animals.filter(animal => animal.continents.includes('Africa'))

const weightOfAllAnimalsInAfrica = allAnimalsInAfrica.map(animal => animal.weight).reduce((prev,curr) => prev + curr)
   

// Hint: As above, but divided by the number of animals in Africa.
const averageWeightOfAllAnimalsInAfrica = weightOfAllAnimalsInAfrica / allAnimalsInAfrica.length
